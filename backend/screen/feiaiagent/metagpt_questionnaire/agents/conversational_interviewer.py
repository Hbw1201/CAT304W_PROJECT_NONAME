...
class ConversationalInterviewerAgent(BaseAgent):
    ...
    def _get_skip_ids(self, answers: Dict[str, str]) -> Set[str]:
        ...
        if answers.get('smoking_history') in ('no', '2'):
            skip_ids.update([
                'smoking_freq',           # 吸烟频率
        ...
        if answers.get('passive_smoking') in ('no', '2'):
            skip_ids.update([
        ...
        if answers.get('kitchen_fumes') in ('no', '2'):
            skip_ids.update([
        ...
        if answers.get('occupation_exposure') in ('no', '2'):
            skip_ids.update([
