...
if (!videoEl) { // [ADD]
  ...
  // 将视频元素插到音频元素后面，保持布局紧邻（可按需调整�?  if (audioEl && audioEl.parentNode) { // [ADD]
...
// 显示TTS播放指示�?function showTTSIndicator(status) {
  ttsStatus.textContent = status;
  ttsIndicator.style.display = "flex";
  log(`显示TTS指示�? ${status}`);
}

// 隐藏TTS播放指示�?function hideTTSIndicator() {
...
// [ADD] 统一设置媒体资源（视频和TTS音频同时播放），并控制显�?隐藏
function setTTSAndVideo(ttsUrl, videoUrl, playLabel) {
  ...
  // 设置TTS音频（有声音�?  if (ttsUrl) {
    ...
    // 添加音频加载事件监听�?    audioEl.onloadstart = () => log('TTS音频开始加�?);
    audioEl.oncanplay = () => log('TTS音频可以播放');
    audioEl.oncanplaythrough = () => log('TTS音频可以流畅播放');
    audioEl.onerror = (e) => log(`TTS音频加载错误: ${e.message || '未知错误'}`);
    audioEl.onabort = () => log('TTS音频加载被中�?);
    audioEl.onstalled = () => log('TTS音频加载停滞');
    audioEl.onwaiting = () => log('TTS音频等待数据');
    audioEl.onplay = () => log('TTS音频开始播�?);
    audioEl.onpause = () => log('TTS音频暂停');
    audioEl.onended = () => log('TTS音频播放结束');
  }

  // 显示播放指示�?  if (videoUrl || ttsUrl) {
    showTTSIndicator(playLabel || "正在播放数字人视频和语音...");
  } else {
    hideTTSIndicator();
  }

  // 返回音频元素作为主要控制元素（因为TTS音频决定播放时长�?  return ttsUrl ? audioEl : (videoUrl ? videoEl : null);
}

// [ADD] 播放并在结束后自动开始录音（视频和TTS音频同步播放�?async function playWithAutoRecord(mediaEl, afterLabel = "语音播放完成，自动开始录�?..") {
  if (!mediaEl) return;
  
  // 添加调试信息
  log(`开始播放媒�? ${mediaEl.tagName}, src: ${mediaEl.src}`);
  
  try {
    // 同时播放视频和音�?    const playPromises = [];
    ...
    // 同时播放视频（如果存在且不是主要元素�?    if (videoEl && videoEl.src && videoEl !== mediaEl) {
    ...
    // 当TTS音频结束时，停止视频并开始录�?    if (mediaEl && mediaEl.tagName === 'AUDIO') {
      ...
      // 如果是视频，当视频结束时开始录�?      mediaEl.onended = () => {
        hideTTSIndicator();
        statusEl.textContent = `状态：${afterLabel}`;
        log(`视频播放完成，准备开始录音`);
    ...
    statusEl.textContent = "状态：媒体播放失败，但内容已显�?;
    
    // 如果是自动播放策略错误，提示用户手动播放
    if (e.name === 'NotAllowedError') {
      log('浏览器阻止了自动播放，请手动点击播放按钮');
      statusEl.textContent = "状态：请手动点击播放按钮开始播�?;
    }
  }
}
...
function restartConversation() {
  ...
  // 隐藏问题信息和进�?  document.getElementById("questionInfo").style.display = "none";
  document.getElementById("progressInfo").style.display = "none";

      // 重置按钮状�?    document.getElementById("btnStart").disabled = false;
    // 录音按钮现在是自动的，不需要手动设�?
  // 隐藏重新开始按�?  btnRestart.style.display = "none";
  ...
  // 重置状�?  isLocalQuestionnaire = false;
  isAgentMode = false;
  isMetaGPTMode = false;
  currentQuestionInfo = null;
...
function unlockAudio() {
  // 创建一个静音的音频元素来解锁播放权�?  const unlockAudio = document.createElement('audio');
  unlockAudio.muted = true;
  unlockAudio.volume = 0;
  
  // 添加一个点击事件监听器到整个页�?  document.addEventListener('click', function unlockAudioOnClick() {
...
async function startConversation() {
  try {
    ...
    // [MOD] 优先使用视频，其次音�?    const mediaEl = setTTSAndVideo(data.tts_url, data.video_url, "正在播放问题..."); // [MOD]
...
async function startMetaGPTQuestionnaire() {
  try {
    ...
    // 统一音视频播�?    const mediaEl = setTTSAndVideo(data.tts_url, data.video_url, "正在播放问题...");
...
async function submitMetaGPTAnswer(text) {
  try {
    ...
    const playLabel = data.invalid_answer ? "答案不清晰，正在重播问题..." : (data.is_complete ? "正在播放评估结果..." : "正在播放新问�?..");
    ...
    if (data.is_complete) {
      ...
      if (mediaEl) {
        ...
        try {
          ...
          hideTTSIndicator();
          statusEl.textContent = "状态：媒体播放失败，但报告已显�?;
        }
      }
    } else {
      if (data.invalid_answer) {
        addToHistory('warning', `回答无效�?{data.invalid_reason || '不够具体/未识�?}`);
      }
      addToHistory('question', `[MetaGPT] ${question}`);
      if (mediaEl) {
        statusEl.textContent = "状态：正在播放新问题语�?视频...";
        try {
          await playWithAutoRecord(mediaEl);
        } catch (e) {
          log(`自动播放失败: ${e.message}`);
...
async function initMetaGPT() {
  try {
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || '初始化失�?);
    log(`�?MetaGPT 初始�? ${data.initialized ? '成功' : '失败'}`);
    statusEl.textContent = `状态：MetaGPT初始�?{data.initialized ? '成功' : '失败'}`;
  } catch (e) {
    log(`�?MetaGPT 初始化失�? ${e.message}`);
    statusEl.textContent = '状态：MetaGPT 初始化失�?;
  }
}
...
async function runMetaGPTDemo() {
  try {
    log("🎯 运行 MetaGPT 演示工作�?..");
    statusEl.textContent = '状态：运行 MetaGPT 演示�?..';
...
async function submitAnswerText(text) {
  try {
    ...
    log(`  - question内容预览: ${data.question ? data.question.substring(0, 100) + '...' : '�?}`);

    sessionId = data.session_id;
    const question = data.question || "(�?";
    qEl.textContent = question;

    // [MOD] 优先使用视频，其次音�?    const mediaEl = setTTSAndVideo(data.tts_url, data.video_url, data.is_complete ? "正在播放评估结果..." : "正在播放新问�?.."); // [MOD]
    ...
    if (data.is_complete) {
      ...
      // 检查是否是评估报告（多种关键词匹配�?      const isReport = question.includes("肺癌早筛风险评估报告") ||
      ...
      if (isReport) {
        ...
        try {
          ...
          // 如果Markdown解析失败，直接显示文�?          reportHtml = question.replace(/\n/g, '<br>');
        ...
        log(`报告类型: ${isReport ? '评估报告' : '普通回�?}`);

        // [MOD] 统一播放（视频优先），结束后不再自动录音（完成态一般不再录�?        if (mediaEl) {
          showTTSIndicator("正在播放评估报告...");
          statusEl.textContent = "状态：正在播放评估报告...";
          try {
            await mediaEl.play();
        ...
        // 虽然不是明确的评估报告，但可能是其他形式的完成结�?        log(`📝 设置完成结果内容到reportContentEl（非标准报告格式）`);
        ...
        // [MOD] 同样仅播放一次（不再触发自动录音�?        if (mediaEl) {
          ...
          try {
            ...
            hideTTSIndicator();
            log(`媒体播放失败: ${e.message}`);
            statusEl.textContent = "状态：媒体播放失败，但结果已显�?;
          }
        }
      }
    } else {
      log(`�?问卷未完成，继续下一题`);
      // 检查是否是API调用失败
      if (question.includes("智谱AI暂时不可�?) || question.includes("系统暂时不可�?)) {
        log("⚠️ 智谱AI调用失败，请稍后重试");
        statusEl.textContent = "状态：智谱AI暂时不可用，请稍后重�?;
        ...
        // 显示重新开始按�?        btnRestart.style.display = "inline-block";

        return; // 不继续处�?      }

      // 检查是否是Agent流程错误（需要重新询问）
      if (question.includes("Agent流程错误")) {
        log("⚠️ 检测到Agent流程错误，正在重新询问问�?..");
        statusEl.textContent = "状态：正在重新询问问题...";
        statusEl.style.color = "#ffc107";
        statusEl.style.backgroundColor = "#fff3cd";

        addToHistory('warning', "刚才的问题出现了错误，正在重新询�?..");
        qEl.style.color = "#ffc107";

        // [MOD] 统一播放（视�?音频），本分支不自动录音
        if (mediaEl) {
          ...
          try {
            ...
            mediaEl.onended = () => {
              hideTTSIndicator();
              statusEl.textContent = "状态：语音播放完成，等待回�?;
              log("媒体播放完成");
            };
          } catch (e) {
            hideTTSIndicator();
            log(`媒体播放失败: ${e.message}`);
            statusEl.textContent = "状态：媒体播放失败，但问题已显�?;
          }
        }
        return;
      }
      ...
      if (isLocalQuestionnaire) {
        ...
        if (question.includes("Agent流程错误")) {
          ...
          // [MOD] 统一播放（视�?音频），结束后自动录�?          if (mediaEl) {
            showTTSIndicator("正在播放重新询问...");
            statusEl.textContent = "状态：正在播放重新询问...";
            try {
              await mediaEl.play();
              mediaEl.onended = () => {
                hideTTSIndicator();
                statusEl.textContent = "状态：语音播放完成，自动开始录�?..";
                log("媒体播放完成，自动开始录�?);
                setTimeout(() => {
      ...
      // [MOD] 统一播放新问题（视频优先），播放后自动录�?      if (mediaEl) {
        statusEl.textContent = "状态：正在播放新问题语�?视频...";
        // 尝试播放，如果失败则提示用户手动播放
        try {
          await playWithAutoRecord(mediaEl); // [MOD]
...
async function startRecording() {
  try {
    ...
    mediaRecorder.onstop = async () => {
      ...
      log(`录音完成，格�? ${mimeType}, 扩展�? ${fileExtension}`);
    ...
    // 隐藏录音按钮（现在是自动录音�?    document.getElementById("btnRec").style.display = 'none';
    ...
    // 音量检测函�?    const checkSilence = () => {
      if (analyser && dataArray) {
        ...
        // �?00次检测输出一次音量信息（避免日志过多�?        if (!window.volumeLogCounter) window.volumeLogCounter = 0;
        window.volumeLogCounter++;
        if (window.volumeLogCounter % 100 === 0) {
          log(`🔊 当前音量: ${currentVolume.toFixed(2)}, 静音阈�? 100, 静音计时: ${silenceStartTime ? ((Date.now() - silenceStartTime) / 1000).toFixed(1) + 's' : '未开�?}`);
        }
        ...
        log("⚠️ 音频分析器未就绪，无法检测音�?);
      }

      // 继续检�?      window.silenceTimer = requestAnimationFrame(checkSilence);
    };

    // 开始音量检�?    window.silenceTimer = requestAnimationFrame(checkSilence);

    log("录音开始，已启�?.5秒静音自动停止功�?);
  } catch (error) {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // 设置按钮事件监听�?  document.getElementById("btnStart").addEventListener("click", startConversation);
...
async function loadReportsList() {
  try {
    ...
    reportsStatsEl.textContent = '统计加载�?..';
    ...
    reportsStatsEl.textContent = `数量: ${stats.total_reports || 0}，总大�? ${stats.total_size_mb || 0} MB，目�? ${stats.reports_dir || ''}`;
...
// 测试函数：测试评估报告显�?function testAssessmentReport() {
  log("🧪 开始测试评估报告显示功�?);

  // 测试1：检查DOM元素是否存在
  log(`📋 DOM元素检�?`);
  log(`  - assessmentReportEl: ${assessmentReportEl ? '存在' : '不存�?}`);
  log(`  - reportContentEl: ${reportContentEl ? '存在' : '不存�?}`);
  log(`  - reportAudioEl: ${reportAudioEl ? '存在' : '不存�?}`);

  // 测试2：检查当前显示状�?  if (assessmentReportEl) {
    const currentDisplay = assessmentReportEl.style.display;
    log(`  - 当前评估报告显示状�? ${currentDisplay}`);
    log(`  - 当前评估报告可见�? ${assessmentReportEl.offsetParent !== null ? '可见' : '不可�?}`);
  }
  ...
  // 测试5：检查最终状�?  setTimeout(() => {
    if (assessmentReportEl) {
      const finalDisplay = assessmentReportEl.style.display;
      log(`📊 最终状态检�?`);
      log(`  - 显示状�? ${finalDisplay}`);
      log(`  - 可见�? ${assessmentReportEl.offsetParent !== null ? '可见' : '不可�?}`);
...
// 测试函数：测试完成状�?function testCompleteStatus() {
  log("🧪 开始测试完成状态功�?);
  ...
  const mockCompleteData = {
    ...
    video_url: "", // [ADD] 可填入测试视�?URL 体验视频播放
    is_complete: true
  };
  ...
  statusEl.textContent = "状态：测试完成状�?- 问卷已完成，显示总结报告";
