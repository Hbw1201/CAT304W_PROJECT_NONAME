...
class Config:
    def __init__(self):
        ...
        # 方言配置
        self.dialect = DialectConfig.DEFAULT
    ...
    def new_full_client_request(seq: int) -> bytes:  # 添加seq参数
        ...
        payload = {
            ...
            "request": {
                ...
                "language": "zh-CN",
                "dialect": config.dialect,  # 动态方言配置
                "enable_speaker_diarization": False,  # 关闭说话人分离，专注方言识别
                "enable_emotion_recognition": False,  # 关闭情感识别，专注方言识别
                "enable_sentiment_analysis": False   # 关闭情感分析，专注方言识别
            }
        }
...
class AsrWsClient:
    ...
    async def start_realtime_stream(self, audio_queue: asyncio.Queue) -> AsyncGenerator[str, None]:
        """实时流式识别，只保留一个最终的识别结果"""
        async def sender():
            ...
            while True:
                try:
                    # 从队列获取音频数据，超时时间很短，确保实时性
                    audio_data = await asyncio.wait_for(audio_queue.get(), timeout=0.1)
                    audio_buffer.extend(audio_data)
                    total_audio.extend(audio_data)
                    
                    # 当缓冲区达到一定大小时发送（约1000ms的音频）
                    if len(audio_buffer) >= 16000:  # 1000ms at 16kHz
                        # 为音频数据添加WAV头部
                        wav_header = create_wav_header(len(audio_buffer))
                        wav_data = wav_header + bytes(audio_buffer)
                        request = RequestBuilder.new_audio_only_request(
                            self.seq, 
                            wav_data,
                            is_last=False
                        ...
                        await self.conn.send_bytes(request)
                        logger.info(f"发送音频包 {self.seq}, 大小: {len(wav_data)} bytes")
                        self.seq += 1
                        send_count += 1
                        audio_buffer.clear()
                        
                        # 每发送3个包后，发送一个累积的音频包（用于提高识别准确性）
                        if send_count % 3 == 0 and len(total_audio) > 0:
                            wav_header_total = create_wav_header(len(total_audio))
                            wav_data_total = wav_header_total + bytes(total_audio)
                            request_total = RequestBuilder.new_audio_only_request(
                                self.seq, 
                                wav_data_total,
                                is_last=False
                            )
                            await self.conn.send_bytes(request_total)
                            logger.info(f"发送累积音频包 {self.seq}, 大小: {len(wav_data_total)} bytes")
                            self.seq += 1
        ...
        try:
            async for response in self.recv_messages():
                ...
                # 只有在收到最后一个包时才返回最终结果
                if response.is_last_package or response.code != 0:
                    if final_result and not result_sent:
                        result_sent = True
                        yield final_result
                    break
...
def create_wav_header(data_length, sample_rate=16000, channels=1, bits_per_sample=16):
    ...
    header.extend((data_length + 36).to_bytes(4, 'little'))  # 文件大小
...
async def main():
    print("=== 实时语音识别 WebSocket 客户端 ===")
    print("支持实时麦克风输入，流式语音识别")
    print("按 Ctrl+C 停止录音")
    print()
    ...
    seg_duration = 200  # 默认200ms分段
    
    print(f"使用流式识别模式")
    print(f"分段时长: {seg_duration}ms")
    print("开始录音...")
    print("-" * 50)
    ...
    def audio_callback(in_data, frame_count, time_info, status):
        # 简单的音量检测
        import struct
        audio_data = struct.unpack('<' + 'h' * (len(in_data) // 2), in_data)
        rms = (sum(x * x for x in audio_data) / len(audio_data)) ** 0.5
        if rms > 100:
            print(f"🔊 检测到音频，音量: {rms:.1f}")
    ...
    try:
        async with AsrWsClient(url, seg_duration) as client:
            ...
            async def audio_sender():
                ...
                while True:
                    try:
                        ...
                        # 当缓冲区达到一定大小时发送（约2000ms的音频）
                        if len(audio_buffer) >= 32000:  # 2000ms at 16kHz
                            # 为音频数据添加WAV头部
                            wav_header = create_wav_header(len(audio_buffer))
                            wav_data = wav_header + bytes(audio_buffer)
                            request = RequestBuilder.new_audio_only_request(
                            ...
                            print(f"📤 发送音频包 {client.seq}, 大小: {len(wav_data)} bytes, 累积: {len(total_audio)} bytes")
                            client.seq += 1
                            send_count += 1
                            audio_buffer.clear()
                            ...
                            if send_count % 5 == 0:
                                ...
                                await client.conn.send_bytes(request_total)
                                print(f"📤 发送累积音频包 {client.seq}, 大小: {len(wav_data_total)} bytes")
                                client.seq += 1
            ...
            # 接收识别结果
            try:
                async for response in client.recv_messages():
                    print(f"收到响应: {response.to_dict()}")  # 调试信息
                    if response.payload_msg:
                        # 打印识别结果
                        if 'result' in response.payload_msg:
                            result = response.payload_msg['result']
...
def asr_transcribe_file(audio_wav_path: str) -> str:
    """
    火山ASR文件转录接口，与现有ASR接口兼容
    输入：16k/16bit/mono 的 wav 文件路径
    输出：识别文本（一次性返回）
    """
    if not os.path.exists(audio_wav_path):
        logger.error(f"ASR错误: 音频文件不存在: {audio_wav_path}")
        return ""
    
    size = os.path.getsize(audio_wav_path)
    if size == 0:
        logger.error(f"ASR错误: 音频文件为空: {audio_wav_path}")
        return ""
    
    logger.info(f"火山ASR开始处理音频文件: {audio_wav_path}, 大小: {size} bytes")
    
    async def transcribe():
        final_result = ""  # 只存储最终结果
        url = config.huosan_asr_url
        seg_duration = 200  # 200ms分段
        
        try:
            async with AsrWsClient(url, seg_duration) as client:
                async for response in client.execute(audio_wav_path):
                    if response.payload_msg:
                        # 提取识别结果
                        current_text = ""
                        if 'result' in response.payload_msg:
            ...
            logger.error(f"火山ASR识别失败: {e}")
            return ""
        
        logger.info(f"火山ASR识别完成，最终结果: '{final_result}'")
        return final_result
    ...
    try:
        ...
        logger.error(f"火山ASR执行失败: {e}")
        return ""

def set_dialect(dialect: str):
    """
    设置ASR方言类型
    支持的方言: mandarin, shanghai, cantonese, taiwanese, sichuan, northeast
    """
    config.set_dialect(dialect)
...
async def start_realtime_asr(audio_queue: asyncio.Queue, result_callback=None):
    """
    启动实时火山ASR识别，只返回一个最终结果
    参数:
        audio_queue: 音频数据队列
        result_callback: 识别结果回调函数，只接收最终的识别文本
    """
    url = config.huosan_asr_url
    seg_duration = 200  # 200ms分段
    ...
    try:
        async with AsrWsClient(url, seg_duration) as client:
            ...
            # 启动实时音频流处理，只获取最终结果
            async for final_text in client.start_realtime_stream(audio_queue):
                if result_callback and final_text:
                    # 只调用一次回调，传递最终结果
                    result_callback(final_text)
                    break  # 只处理第一个（也是唯一的）结果
                    
    except Exception as e:
        logger.error(f"实时火山ASR识别失败: {e}")
        if result_callback:
            result_callback("")
