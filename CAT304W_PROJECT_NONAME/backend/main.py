...
def ensure_chat_service() -> None:
    ...
    if not AUTO_START_CHAT:
        logger.info("[chat] AUTO_START_CHAT=0, skipping auto-launch")
        return
    logger.info("[chat] AUTO_START_CHAT=1, auto-launch enabled")

    if is_chat_running():
        logger.info("[chat] chat service already running")
        return
    ...
    if _is_port_open(target_host, target_port):
        logger.warning("[chat] port %s:%s already in use; skipping auto-start", target_host, target_port)
        return

    proc = start_chatbot_node()
    if not proc:
        logger.error("[chat] failed to start chat service")
        return
    _chat_process = proc
    ...
    while time.time() < deadline:
        if is_chat_running():
            logger.info("[chat] chat service started")
            return
        time.sleep(0.3)
    logger.error("[chat] chat service failed to start within 8s")
...
print(f"[chat] external mode: expecting chat service at {_chat_base}")
